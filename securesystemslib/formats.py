...
HASHDICT_SCHEMA = SCHEMA.DictOf(
    key_schema=SCHEMA.AnyString(), value_schema=HASH_SCHEMA
)
...
def _create_gpg_pubkey_with_subkey_schema(pubkey_schema):
    ...
    subkey_schema_tuple = (
        "subkeys",
        SCHEMA.Optional(
            SCHEMA.DictOf(key_schema=KEYID_SCHEMA, value_schema=pubkey_schema)
        ),
    )
...
_GPG_DSA_PUBKEY_SCHEMA = SCHEMA.Object(
    ...
    keyid=KEYID_SCHEMA,
    keyval=SCHEMA.Object(
        public=GPG_DSA_PUBKEYVAL_SCHEMA, private=SCHEMA.String("")
    ),
)
...
_GPG_ED25519_PUBKEY_SCHEMA = SCHEMA.Object(
    ...
    keyid=KEYID_SCHEMA,
    keyval=SCHEMA.Object(
        public=GPG_ED25519_PUBKEYVAL_SCHEMA, private=SCHEMA.String("")
    ),
)
...
SIGNATURE_SCHEMA = SCHEMA.Object(
    object_name="SIGNATURE_SCHEMA", keyid=KEYID_SCHEMA, sig=HEX_SCHEMA
)

# A dict where the dict keys hold a keyid and the dict values a key object.
KEYDICT_SCHEMA = SCHEMA.DictOf(key_schema=KEYID_SCHEMA, value_schema=KEY_SCHEMA)

ANY_SIGNATURE_SCHEMA = SCHEMA.OneOf([SIGNATURE_SCHEMA, GPG_SIGNATURE_SCHEMA])
...
# the gpg keyring)
ANY_VERIFICATION_KEY_SCHEMA = SCHEMA.OneOf([ANYKEY_SCHEMA, GPG_PUBKEY_SCHEMA])

VERIFICATION_KEY_DICT_SCHEMA = SCHEMA.DictOf(
    key_schema=KEYID_SCHEMA, value_schema=ANY_VERIFICATION_KEY_SCHEMA
)

ANY_KEYDICT_SCHEMA = SCHEMA.OneOf(
    [KEYDICT_SCHEMA, VERIFICATION_KEY_DICT_SCHEMA]
)

ANY_PUBKEY_SCHEMA = SCHEMA.OneOf([PUBLIC_KEY_SCHEMA, GPG_PUBKEY_SCHEMA])

ANY_PUBKEY_DICT_SCHEMA = SCHEMA.DictOf(
    key_schema=KEYID_SCHEMA, value_schema=ANY_PUBKEY_SCHEMA
)
