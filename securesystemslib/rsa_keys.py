...
def create_rsa_signature(private_key, data, scheme="rsassa-pss-sha256"):
    ...
    try:
        ...
        if scheme.startswith("rsassa-pss"):
            ...
            # exceptions raised by pyca/cryptography.
            signature = private_key_object.sign(
                data, padding.PKCS1v15(), digest_obj.algorithm
            )
    ...
    try:
        ...
        try:
            if signature_scheme.startswith("rsassa-pss"):
                ...
                public_key_object.verify(
                    signature, data, padding.PKCS1v15(), digest_obj.algorithm
                )
...
def create_rsa_public_and_private_from_pem(pem, passphrase=None):
  ...
  """
    ...
    try:
        private_key = load_pem_private_key(
            pem.encode("utf-8"), passphrase, backend=default_backend()
        )
...
def encrypt_key(key_object, password):
  ...
  """
    ...
    if (
        "private" not in key_object["keyval"]
        or not key_object["keyval"]["private"]
    ):
...
def _encrypt(key_data, derived_key_information):
    ...
    encryptor = Cipher(
        algorithms.AES(symmetric_key), modes.CTR(iv), backend=default_backend()
    ).encryptor()

    # Encrypt the plaintext and get the associated ciphertext.
    # Do we need to check for any exceptions?
    ciphertext = (
        encryptor.update(key_data.encode("utf-8")) + encryptor.finalize()
    )
    ...
    hmac_object = hmac.HMAC(
        symmetric_key, hashes.SHA256(), backend=default_backend()
    )
...
def _decrypt(file_contents, password):
    ...
    try:
        salt, iterations, read_hmac, iv, ciphertext = file_contents.split(
            _ENCRYPTION_DELIMITER
        )
    ...
    # The decryption routine may verify a ciphertext without having to perform
    # a decryption operation.
    generated_hmac_object = hmac.HMAC(
        symmetric_key, hashes.SHA256(), backend=default_backend()
    )
