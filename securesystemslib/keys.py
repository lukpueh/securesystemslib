...
def format_metadata_to_key(
    key_metadata, default_keyid=None, keyid_hash_algorithms=None
):
    """
  <Purpose>
...
def _get_keyid(keytype, scheme, key_value, hash_algorithm="sha256"):
    ...
    key_meta = format_keyval_to_metadata(
        keytype, scheme, key_value, private=False
    )
...
def verify_signature(
    ...
    if keytype == "rsa":
        if scheme in RSA_SIGNATURE_SCHEMES:
            valid_signature = rsa_keys.verify_rsa_signature(
                sig, scheme, public, data
            )
        ...
        if scheme == "ed25519":
            ...
            valid_signature = ed25519_keys.verify_signature(
                public, scheme, sig, data
            )
        ...
        if scheme in ["ecdsa-sha2-nistp256", "ecdsa-sha2-nistp384"]:
            valid_signature = ecdsa_keys.verify_signature(
                public, scheme, sig, data
            )
