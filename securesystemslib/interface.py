...
def _generate_and_write_rsa_keypair(
    filepath=None, bits=DEFAULT_RSA_KEY_BITS, password=None, prompt=False
):
...
def generate_and_write_rsa_keypair(
    password, filepath=None, bits=DEFAULT_RSA_KEY_BITS
):
    """Generates RSA key pair and writes PEM-encoded keys to disk.
...
def generate_and_write_rsa_keypair_with_prompt(
    filepath=None, bits=DEFAULT_RSA_KEY_BITS
):
    """Generates RSA key pair and writes PEM-encoded keys to disk.
...
def generate_and_write_unencrypted_rsa_keypair(
    filepath=None, bits=DEFAULT_RSA_KEY_BITS
):
    """Generates RSA key pair and writes PEM-encoded keys to disk.
...
def import_rsa_privatekey_from_file(
    filepath,
    password=None,
...
def import_rsa_publickey_from_file(
    filepath, scheme="rsassa-pss-sha256", storage_backend=None
):
    ...
    try:
        ...
        raise exceptions.Error(
            "Cannot import improperly formatted" " PEM file." + repr(str(e))
        )

    return rsakey_dict


def _generate_and_write_ed25519_keypair(
    filepath=None, password=None, prompt=False
):
...
def import_ed25519_privatekey_from_file(
    filepath, password=None, prompt=False, storage_backend=None
):
...
def import_privatekey_from_file(
    filepath, key_type=None, password=None, prompt=False
):
