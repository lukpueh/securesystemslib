...
def _get_key_file_decryption_password(password, prompt, path):
  """Decryption password prompt helper. """

  # Allow either pw or prompt so we don't have to decide which takes precedence
  if password is not None and prompt:
    raise ValueError("passing 'password' and 'prompt=True' is not allowed")

  # Prompt for pw and treat empty pw as no pw. On the prompt one can only
  # indicate that decryption is unneeded by entering no pw, i.e. empty string.
  if prompt:
  ...
  # Only check that passed or entered password is a string. We don't care about
  # the strength (e.g. length) of the decryption password.
  if password is not None:
    securesystemslib.formats.PASSWORD_SCHEMA.check_match(password)
...
def import_public_keys_from_file(filepaths, key_types=None):
  """
  <Purpose>
    Takes a list of filepaths to public keys and returns them as a dictionary
    conformant with 'securesystemslib.formats.KEYDICT_SCHEMA'.

  <Arguments>
    filepaths:
      List of paths to public key files.

    key_types: (optional)
      List of types of each of the keys to be imported, associated by index.
      Must be one of KEY_TYPE_RSA, KEY_TYPE_ED25519 or KEY_TYPE_ECDSA.
      If not specified, all keys are assumed to be RSA.

  <Exceptions>
    securesystemslib.exceptions.FormatError, if 'key_types' is passed but does
    not have the same length as filepaths
